...
//        $password_reset_link = url('/') . route('agent-password-reset-form', [
//                'token' => $token
//            ], false);

        $baseUrl = env('PUBLIC_BASE_URL'); // Fetch the base URL from environment
    ...
    {
        return Password::broker('agents'); // Use the 'agents' broker if you have a separate guard or broker for agents
    }
    ...
    public function passwordResetForm()
    {
        ...
        return view('password.set_password', compact('token'));
    }
    ...
    public function resetPassword($requestData)
    {
        ...
        $user = Agent::whereEmail($email)->firstOrFail();
